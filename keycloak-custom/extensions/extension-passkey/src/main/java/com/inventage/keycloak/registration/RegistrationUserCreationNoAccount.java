...
/**
 * This class extends the built-in RegistrationUserCreation class. We modified
 * the following functionalities:
 * (1): We do not create a user account immediately after the user fills out the
 * form correctly. We want to delay the creation
 * as long as possible, because it is possible that a user cancels the
 * registration just before setting up a password or passkey.
 * <p>
 * (2): We added a check to make sure that an email has not already been used
 * for another account.This check would normally be done
 * in the built-in RegistrationProfile class. The reason we don't just add the
 * built-in RegistrationProfile class to the registration
 * flow is that it presumes that the user account is already set/created.
 * <p>
 * IMPORTANT: This FormAction should not be used in combination with other
 * built-in FormActions ("or Authenticators") as most of them
...
public class RegistrationUserCreationNoAccount extends RegistrationUserCreation {
    ...
    // Copied from org.keycloak.authentication.forms.RegistrationProfile as the
    // validation check of RegistrationUserCreation does not check for invalid
    // emails.
    @Override
    public void validate(ValidationContext context) {
        ...
        // We check if the email address is already in use. If yes, we return an error
